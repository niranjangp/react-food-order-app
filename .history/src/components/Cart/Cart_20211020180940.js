...
const Cart = (props) => {
    ...
    const submitOrderData = async (userData) => {
        try {
            ...
            //old const resp = await fetch('https://react-http-aafdf-default-rtdb.firebaseio.com/orders.json', {
            const resp = await fetch('https://react-food-order-ab2e8-default-rtdb.firebaseio.com/orders.json', {
                method: 'POST',
                body: JSON.stringify({
                    user: userData,
